      ...
      <form class="login-form" @submit.prevent="register">
        ...
        <div v-if="errors.length" class="row">          
          <div  class="input-field col s12 m12 l12">
              <p  v-for="(error, index) in errors" :key="index" class="margin left-align medium-small red-text text-darken-1">{{error}}</p>
          </div>          
        </div>
        
        <div class="row">
          <div class="input-field col s12">
               <button class="btn waves-effect waves-light col s12" >Registrarme</button>  
          </div>
        </div>
